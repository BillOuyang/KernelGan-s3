...
class Config:
    def __init__(self):
        ...
        self.parser.add_argument('--scale_factor', type=float, default=1/3, help='The downscaling scale factor')  #TODO change to 0.5
        self.parser.add_argument('--X4', action='store_true', help='The wanted SR scale factor')

        # Network architecture
        self.parser.add_argument('--G_chan', type=int, default=64, help='# of channels in hidden layer in the G')  #TODO 64
        self.parser.add_argument('--D_chan', type=int, default=64, help='# of channels in hidden layer in the D')
        self.parser.add_argument('--G_kernel_size', type=int, default=13, help='The kernel size G is estimating') #TODO change to 13
        self.parser.add_argument('--D_n_layers', type=int, default=9, help='Discriminators depth')  ##TODO change to 7
        self.parser.add_argument('--D_kernel_size', type=int, default=7, help='Discriminators convolution kernels size')  ##TODO change to 7

        # Iterations
        self.parser.add_argument('--max_iters', type=int, default=4000, help='# of iterations')  ##TODO 3000

        # Optimization hyper-parameters
        self.parser.add_argument('--g_lr', type=float, default=2e-4, help='initial learning rate for generator')
        self.parser.add_argument('--d_lr', type=float, default=2e-3, help='initial learning rate for discriminator')
    ...
    def parse(self, args=None):
        ...
        self.conf.G_structure = [7,5,3,1]   ###TODO change back to [7,5,3,1,1,1]
        print("Scale Factor: %s \tZSSR: %s \tReal Image: %s" % (('X4' if self.conf.X4 else 'X3'), str(self.conf.do_ZSSR), str(self.conf.real_image)))
        return self.conf
