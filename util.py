...
def save_final_kernel(k_2, conf):
    ...
    plt.imshow(k_2, cmap='gray')
    path = os.path.join(conf.output_dir_path, '%s_kernel_x2.png' % conf.img_name)
    plt.savefig(path)
    if conf.X4:
        k_4 = analytic_kernel(k_2)
        plt.imshow(k_4, cmap='gray')
        path = os.path.join(conf.output_dir_path, '%s_kernel_x4.png' % conf.img_name)
        plt.savefig(path)
...
def run_zssr(k_2, conf):
    ...
    if conf.do_ZSSR:
        ...
        sr = ZSSR(conf.input_image_path, scale_factor=3, kernels=[k_2]).run()  ##TODO change to 2
        max_val = 255 if sr.dtype == 'uint8' else 1.
        plt.imsave(os.path.join(conf.output_dir_path, 'ZSSR_'+ conf.img_name+'.png'), sr, vmin=0, vmax=max_val, dpi=1)
