...
scale_factor = np.array([3, 3])  # choose scale-factor
...
noise_level = 0.01  # this option allows deviation from just a gaussian, by adding multiplicative noise noise
images_path = '.\input_image\*HR.jpg'
output_path = '.\output_kernel'
...
for i, path in enumerate(glob.glob(images_path)):
    im_name = ntpath.basename(path).split(".")[0]
    im = img.imread(path)
    ...
    path = output_path+"\kernal_"+im_name+'.png'
    # plt.imsave(path, arr=kernel, cmap='gray')
    # plt.show()
    plt.figure()
    plt.imshow(kernel, cmap='gray')
    plt.savefig(path)
